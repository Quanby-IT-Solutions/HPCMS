...
export default function SupervisorPatientsPage() {
	...
	const typedData = data as { patients?: Array<Record<string, unknown>> } | undefined
	const patients = typedData?.patients ?? []
					...
					<TableBody>
						{patients.map(p => (
							<TableRow key={p.id as string} className="cursor-pointer">
								...
								<TableCell className="text-sm">{(p.sexAtBirth as string) ?? "—"}</TableCell>
							</TableRow>
						))}
					</TableBody>
				</Table>
